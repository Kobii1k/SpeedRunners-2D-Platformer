...
public class Player extends MapObject {
	...
	private final int[] NUMFRAMES = { 1, 9, 5, 3, 3, 5, 4, 2, 1, 3 };
	private final int[] FRAMEWIDTHS = { 40, 48, 80, 40, 40, 40, 80, 40, 40, 40 };
	private final int[] FRAMEHEIGHTS = { 40, 40, 40, 40, 40, 80, 40, 40, 40, 40 };
	private final int[] SPRITEDELAYS = { -1, 4, 2, 20, 20, 2, 2, 1, -1, 1 };
	...
	private static final int TELEPORTING = 9;

	// emotes (ne fonctionne pas)
	...
	public Player(TileMap tm) {
		...
		//setup des valeurs de base du personnage
		...
		// try catch pour récupérer une erreur et sa provenance
		try {
			...
			int count = 0;
			sprites = new ArrayList<BufferedImage[]>(); //liste d'images sprites
			for (int i = 0; i < NUMFRAMES.length; i++) { //Comme dans "HUD.java" on récupère des parcelles d'images à partir d'une seule grande image.
	...
	//getters et setters
	public void init(ArrayList<Enemy> enemies, ArrayList<EnergyParticle> energyParticles) {this.enemies = enemies;}
	...
	public void hit(int damage) { //flinching == you got hit == invincible for around 1 sec + knockback
		if (flinching)
			return;
		stop();
		health -= damage;
	...
	private void getNextPosition() { //gérer le déplacement du personnage dans toute les situations
		...
		// vecteurs de mouvements et vitesse
		...
		// ne pas se déplacer en attaquant, sauf en l'air
		...
		// la tombée
		if (falling) { // si l'on appuie moins longtemps sur la touche de saut, on saute moins haut. :)
			dy += fallSpeed;
			if (dy < 0 && !jumping)
				dy += stopJumpSpeed;
			if (dy > maxFallSpeed)
	...
	private void setAnimation(int i) { //changer l'animation du personnage et sa taille (hitbox & hurtbox)
		currentAction = i;
	...
	public void update() {
		...
		//Si l'ennemi attaque
		//non fonctionnel, les ennemis n'attaquent pas pour le moment.
		//seul les collisions fonctionnent
		...
		// setup priorités dans les animations
		if (teleporting) {
	...
	public void draw(Graphics2D g) {
		...
		// affichage emote => non fonctionnel
		if (emote == CONFUSED) {
